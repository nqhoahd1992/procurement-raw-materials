...
- **Recipient:** the person who must receive the goods (GR/SFU assignee). **If the task has been delegated to someone else, both the assignee and the original Requester receive the email** (both as `To`, not CC — so the Requester stays aware of who's handling their request). If unassigned (nobody delegated — the Requester is doing it themselves), only the Requester receives it once. Never Procurement.
...
|---|---|---|
| `Goods Receipt & Acceptance` | `GRAssignedToID` | If `Id` present (not blank and not `0`) → `Email` from `Employee List` **and** `RequesterEmail` (both, semicolon-separated in `To`); otherwise → `RequesterEmail` only |
| `Pending Supplier Follow-up`, **no** Execution Log `StepNumber = 4` row yet, or `StepNumber = 4` **and** `StepNumber = 5` both present | `SFU1AssignedToID` | If `Id` present → `Email` from `Employee List` **and** `RequesterEmail` (both); otherwise → `RequesterEmail` only |
...
- **Yes (has assignee — delegated to someone else):** add **SharePoint → Get item** (List `Employee List`, Id = `AssigneeId`); set `varRecipientEmail` = **both** the assignee's `Email` and the request's `RequesterEmail`, joined with `;` so Outlook treats them as two separate `To` recipients:
  ```
  concat(body('Get_item_-_Employee')?['Email'], ';', items('Apply_to_each')?['RequesterEmail'])
  ```
  (adjust the action name inside `body(...)` to match the actual `Get item` action's name in your flow — check **Code view** if unsure). Set `varRecipientName` = the request's `GRAssignedToID`/`SFU1AssignedToID` `Value` (used only for the greeting — the Requester receives the same email as an FYI copy, not personally greeted).
- **No (unassigned — Requester is doing it themselves):** set `varRecipientEmail` = `item()?['RequesterEmail']` only (no duplicate), `varRecipientName` = `item()?['RequesterID']?['Value']`.
...
- **One `Send an email` action per request, but 2 recipients when delegated.** When the task has been assigned to someone else, that single email is addressed to both the assignee and the Requester (`To` field, semicolon-separated) — not two separate sends. If a person is responsible for several requests they still receive one email per request. Grouping into one email per person is possible but more complex; kept 1-per-request for simplicity given the short-email requirement.

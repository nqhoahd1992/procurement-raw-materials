...
Resolved in `App.OnStart` → `gCurrentUser` via `LookUp('RM User', EmployeeID.Id = gCurrentEmployee.ID)`, read as `gCurrentUser.Role.Value` everywhere role-gating is needed (there is no separate `gUserRole` variable — it existed at one point but was never read anywhere, so App Checker flagged it as unused and it was removed). **This list is now the app's membership gate**: an employee must have a matching row here (any `Role`, including `Requester`) or `gCurrentUser` is blank and `HomeScreen` shows the "account not found" message instead of the app — being in `Employee List` alone is no longer enough (that was the old, tenant-wide behavior). There is no synthetic `Requester` fallback anymore; every requester needs a real `'RM User'` row.

**`EmployeeID.Id` filtering here is non-delegable, and deliberately left as-is.** App Checker flags it (same root cause as `RequestID.Id` on the log lists — SharePoint never delegates equality on a Lookup column's `.Id`/`.Value`), but `'RM User'` is a short, hand-maintained roster of the company's actual app users (~10 rows today) with no in-app writer at all — it will never approach the Data row limit the way a transactional log does. A `RequestIDNum`-style fix (a plain Number column mirroring `EmployeeID.Id`) was tried and reverted: with no create-site in `Src/`, that column would have to be entered by hand for every row forever, which is real ongoing maintenance burden for a warning with no realistic path to actually firing. Revisit only if this list ever stops being a small, manually-curated roster.

Because of this gate, `GoodsReceiptScreen.ddAssignReceiver_GR` and `SupplierFollowUpScreen.ddAssignReceiver_SFU` (the "Assign to someone else" pickers) are filtered to only show `Employee List` rows that have an active `'RM User'` row: `Filter(ForAll(Filter('RM User', IsActive) As ru, With({wEmpId: ru.EmployeeID.Id}, LookUp('Employee List', ID = wEmpId))), Not(IsBlank(ID)))`. This prevents assigning a Goods Receipt / Supplier Follow-up task to someone who wouldn't be able to log in to act on it. **Not** `Filter('Employee List', ID in ForAll(...))` — that `in`-against-a-computed-table form is non-delegable; see the delegation note in `CLAUDE.md` under `'RM User'`.
...
| `SupplierSummary` | **Single line of text** ⚠ *(was multiline — must be converted, see below)* | step 1. Column name unchanged, but the UI now labels it **"SO/PO/INV"** everywhere it's shown (`ProcurementExecutionScreen`'s input — now a single-line `TextInput`, `RequestDetailScreen`'s log-detail dialog, `AccountingScreen`'s reference panel) — it holds a Sales/Purchase Order or Invoice number, not a free-text negotiation summary. **Must be unique across requests**: `ProcurementExecutionScreen` refuses to submit when another request's step-1 row already carries the same value (checked twice — on `btnSubmit_PE` before the upload, and again in `formConfirmation.OnSuccess` right before the write, so two simultaneous submits can't both get through). The value is `Trim`ed before both the check and the write, so trailing spaces can't smuggle a duplicate past |
...
**⚠ `SupplierSummary` must be a *single line of text* column, not multiline.** SharePoint refuses to filter on a `Note` (multiple lines of text) column at all — the REST call comes back with *"The field of type 'Note' cannot be used in the query filter expression"* — so the uniqueness `LookUp('RM Procurement Execution Log', StepNumber = 1 && SupplierSummary = …)` on `ProcurementExecutionScreen` cannot work until the type is changed. Fix it in **List settings → `SupplierSummary` → Single line of text**; the existing values are SO/PO/INV numbers well under 255 characters, so nothing is truncated (verify first if any legacy row still holds an old free-text negotiation summary — those *would* be cut at 255). Add an index on the column in the same visit, otherwise the check does a full-list scan on every submit.

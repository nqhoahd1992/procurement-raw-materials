...
- `colRequirementFiles` — `RequestDetailScreen`'s "Requirement Files (Requester)" list (`rowRequirementFiles_RD` / `galRequirementFiles_RD`, placed just above `rowInvoiceLink`). `{FileName, FileURL}`, built in `OnVisible` by splitting `gSelectedRequest.RequirementFiles` on `|` and composing each URL as `gSharePointAttachmentBase & Text(ID) & "/" & EncodeUrl(FileName)`; seeded with a literal in `App.OnStart` for its design-time schema. The row's `Visible`/`Height` and the `CountRows(colRequirementFiles)` term in `rowInfoBlock.LayoutMinHeight` must stay in sync — that container sizes itself by summing its visible children.
  - **`rowInfoBlock` has no flexible child.** Every direct child sets `FillPortions: =0` and a real `Height`, so `LayoutMinHeight` *is* the block's height and must be ≥ the true sum or the last children get clipped — `conScrollable` scrolls, `rowInfoBlock` does not. Its base constant covers the always-visible children (`rowInfo1` 60, `rowInfo2` 90, `rowInfo4` 60, `rowInfo3` 84 — its `LayoutMinHeight` exceeds its `Height`, so the min wins — `rowDescription` 130, `rowOfficialInvoiceLink` 70) plus the 20+20 padding and one 4px `LayoutGap` per child boundary; every conditional child then contributes `4 + <its height>`. `rowGoodsReceipt` was the one exception until it was fixed: it had no `FillPortions`, so it absorbed whatever the (under-counted) base left over and silently clipped its own round-history gallery. It now computes `122 + CountRows(colRoundHistory) * 92 + If(Not(IsBlank(…CreditNote)), 68, 0)`, mirrored in `rowInfoBlock`'s term — change the two together. Don't add a child gated on `ProcurementType = "Invoice Supplied"` expecting a matching term: the only Invoice-Supplied gate on this screen is `colInvoiceType`, a *column* inside horizontal `rowInfo3`, which changes no height at all.
